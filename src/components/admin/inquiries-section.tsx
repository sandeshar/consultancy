...
const InquiriesSection = () => {
    ...
    const fetchContacts = useCallback(async () => {
        ...
        try {
            ...
            const response = await fetch(`/api/admin/contacts?status=${currentFilter}&page=${currentPage}&limit=${itemsPerPage}${searchParam}`);
            const data = await response.json();

            if (data.success) {
                console.log('Received data:', data);
                console.log('Contact stats:', data.counts);
    ...
    }, [currentFilter, currentPage, searchTerm]);

    useEffect(() => {
        fetchContacts();
    }, [fetchContacts]);

    const handleFilterChange = (filter: 'all' | 'unseen' | 'processing' | 'resolved') => {
